...
/**
 * Always composed inside a base Section. The kicker is the only place a
...
export default function PageHero({ kicker, heading, lead, cta }: PageHeroProps) {
  return (
    <div>
      <p className="mb-3 text-sm text-accent">{kicker}</p>
      <h1 className="max-w-[18ch]">{heading}</h1>
      <p className="mt-4 max-w-[46ch] text-muted">{lead}</p>
      {cta ? (
        <div className="mt-8">
